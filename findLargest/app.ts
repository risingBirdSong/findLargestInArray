...
function delayer(speed: number) {
  return new Promise((resolve, reject) => {
    setTimeout(() => resolve("done!"), speed);
  });
}

async function findLargest(numbers: number[], howMuchDelay: number) {
  let max = numbers[0];
...
function ifUserEntersCommaSeparatedValues(this: any) {
  //@ts-ignore
  let userInput = <string>$(this)
    .siblings("input")
    .val();
...
function theUserPassedAnArray(that: any, vals: string) {
  $(".default").html(vals);
}
